...
int verify_blockchain(const Blockchain* chain) {
    ...
    // Verify genesis block first
    const Block* genesis = &chain->blocks[0];
    char computed_hash[HASH_SIZE];
    calculate_block_hash(genesis, computed_hash);
    
    if (strcmp(genesis->hash, computed_hash) != 0) {
        print_error("Genesis block has invalid hash!");
        return 0;
    }
    ...
    for (int i = 1; i < chain->block_count; i++) {
        ...
        if (strcmp(block->hash, computed_hash) != 0) {
            printf(COLOR_RED "✗ Invalid hash in block %d!\n" COLOR_RESET, block->index);
            return 0;
        }

        if (strcmp(block->previous_hash, chain->blocks[i-1].hash) != 0) {
            printf(COLOR_RED "✗ Invalid previous hash in block %d!\n" COLOR_RESET, block->index);
            return 0;
        }
    }

    print_success("Blockchain verification passed!");
    return 1;
}
